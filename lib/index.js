...
function applyDefaults (options) {
	...
	if (!options.replicaSetName)		{ options.replicaSetName = null; };
...
function connect(obj) {
	...
	if (obj.options.username && obj.options.password) {
		...
		if (obj.options.replicaSetName !== null)
			connectionStringOptions += "&replicaSet="+obj.options.replicaSetName;
	}
	...
	if (connectionStringOptions.length === 1) {
		connectionStringOptions = "";
	}

	return MongoOplog(connectionString+connectionStringOptions, this.options);
}
